...
enum PipePktType : uint32_t
{
    ...
    PIPE_PKT_UPLOAD_JT1078_DEVICE_ID, // 添加新的device_id，在获取到数据包之后，解析得到device_id后发送到主线程保存
    PIPE_PKT_KICK_DEVICE_ID,          // device_id互踢

    PIPE_PKT_FORWARD_JT1078, // 转发jt1078包
                             // pipe_header
                             // pipe_body
                             //      device_id(uint64_t)类型
                             //      pipe_other_data
    ...
    PIPE_PKT_UPLOAD_PC_DEVICE_ID,//添加pc想要的device_id数据
};
